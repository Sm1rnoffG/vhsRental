...
sealed class OrderActions {
    ...
    data class OnMyOrderDetailRequest(val orderAndMovie: Pair<DomainOrder, DomainMovie>) : OrderActions()
    ...
    fun emitOrderAction(action: OrderActions) {
        when (action) {
            ...
            is OrderActions.OnLoadMyList ->
                _uiStateFlow.update { OrderUiState.OrderList(getAllOrders().filter { it.user == action.userId }) }
            is OrderActions.OnMyOrderDetailRequest ->
                _uiStateFlow.update { OrderUiState.MyOrderDetail(action.orderAndMovie.first, action.orderAndMovie.second) }
        }
    }
